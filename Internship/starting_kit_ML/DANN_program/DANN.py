...
class label_classifier(Layer) :
    def __init__(self,num_classes,name):
        ...
        self.hidden_ftr_1 = Dense(32, activation='relu', name="feature_ext_1")
        self.hidden_lbl_1 = Dense(32, activation='relu', name="label_clf_1")
        self.hidden_lbl_2 = Dense(32, activation='relu', name="label_clf_2")
...
class domain_classifier(Layer) :
    def __init__(self,num_domains,name):
        ...
        self.hidden_dmn_1 = Dense(32, activation='relu', name="domain_clf_1")
...
def build_training_dataset_for_DANN (x_source, y_source, x_target, num_epochs, half_batch_size, portion_of_target_for_train=1) :
    nb_of_source_events = x_source.shape[0]
    nb_of_target_events = x_target.shape[0]
    
    nb_target_for_train = int(nb_of_target_events*portion_of_target_for_train)
    ...
    source_dataset = source_dataset.shuffle(buffer_size=nb_of_source_events).repeat(count=num_epochs)
    ...
    target_dataset = target_dataset.shuffle(buffer_size=nb_target_for_train).repeat(count=-1)
...
def build_datasets_for_DANN (x_sources,y_sources,x_targets,y_targets,num_epochs,batch_size,portion_of_target_for_train=1) :
    ...
    for x_source,y_source,x_target in zip (x_sources,y_sources,x_targets) :
        training_dataset = build_training_dataset_for_DANN(x_source, y_source, x_target, num_epochs, half_batch_size, portion_of_target_for_train)
...
# ================================
# Adapt data for 3D DANN
# ================================

def process_datasets(features,labels,weights):
    batch_size = tf.shape(features)[0]
    features = tf.concat([features, tf.reshape(tf.cast(labels["domain_preds"], dtype=tf.float32),shape=(batch_size,1))], axis=1)
    return (features,labels,weights)

def add_new_feature(training_datasets,testing_datasets,x_sources,x_targets):
    training_datasets_3D,testing_datasets_3D = [],[]
    x_sources_3D,x_targets_3D = [],[]
    
    for training_dataset,testing_dataset,x_source,x_target in zip(training_datasets,testing_datasets,x_sources,x_targets) :
        training_datasets_3D.append(training_dataset.map(process_datasets))
        
        testing_datasets_3D.append(testing_dataset.map(process_datasets))
        
        x_source_3D = x_source.copy()
        x_source_3D["x3"] = 1.0
        x_sources_3D.append(x_source_3D)

        x_target_3D = x_target.copy()
        x_target_3D["x3"] = 0.0
        x_targets_3D.append(x_target_3D)

    return training_datasets_3D,testing_datasets_3D,x_sources_3D,x_targets_3D