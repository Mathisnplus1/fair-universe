...
def visualize_DANN_summary(model) :
    ...
    section_colors = {
    'feature_ext_1 ' : f'{Fore.green}',
    'gradient_rev ' : f'{Fore.hot_pink_1a}',
    'domain_clf_1 ' : f'{Fore.hot_pink_1a}',
    'domain_preds ' : f'{Fore.hot_pink_1a}',
    'label_clf_1 ' : f'{Fore.blue}',
    'class_preds ' : f'{Fore.blue}'
    }
    for i in range(2,20) :
        section_colors[f'domain_clf_{i} '] = f'{Fore.hot_pink_1a}'
        section_colors[f'label_clf_{i} '] = f'{Fore.blue}'

    for line in summary:
        for section, color in section_colors.items():
            if section in line:
                print(color + line + Style.reset)
                break
        else:
            print(line)

def visualize_DANN_boundaries(model, X_s, y_s, X_t, y_t, settings, scores,
                              before_training=False, # Show true color of the points if True and prediction colors if False
    ...
    if not before_training :
        if regions_alpha > 0 :
            ...
            axs[1].contourf(xx, yy, Z, alpha=regions_alpha,cmap="bwr")

            # Add legend elements
            legend_elements.append(Patch(facecolor='red', label='Signal region', alpha=regions_alpha))
            legend_elements.append(Patch(facecolor='blue', label='Background region', alpha=regions_alpha))

    # Define a norm for proper cmapping of the points
    ...
    if source_alpha > 0 :
        if before_training :
            axs[1].scatter(X_s[:, 0], X_s[:, 1], c=y_s, cmap="bwr", edgecolors='k',alpha=source_alpha,marker="s")
            legend_elements.append(Line2D([0],[0],marker='.',ls="",markeredgecolor="b",markerfacecolor='b',label='Background point',alpha=source_alpha))
            legend_elements.append(Line2D([0],[0],marker='.',ls="",markeredgecolor="r",markerfacecolor='r',label='Signal point',alpha=source_alpha))
        else :
            y_pred_s = np.argmax(model.predict(X_s)[0],axis=1)
            axs[1].scatter(X_s[:, 0], X_s[:, 1], c=y_s-y_pred_s, cmap="bwr", norm=norm, edgecolors='k',alpha=source_alpha,marker="s")
            legend_elements.append(Line2D([0],[0],marker='.',ls="",markeredgecolor="w",markerfacecolor='w',label='Right classification',alpha=1))
            legend_elements.append(Line2D([0],[0],marker='.',ls="",markeredgecolor="r",markerfacecolor='r',label='Wrong background',alpha=source_alpha))
            legend_elements.append(Line2D([0],[0],marker='.',ls="",markeredgecolor="b",markerfacecolor='b',label='Wrong signal',alpha=source_alpha))
        legend_elements.append(Line2D([0],[0],marker='s',ls="",markeredgecolor="black",markerfacecolor='w',label='Source points',alpha=source_alpha))
            
    # Plot the target data points
    if target_alpha > 0 :
        if before_training :
            if target_colors :
                axs[1].scatter(X_t[:, 0], X_t[:, 1], c=y_t, cmap="bwr", edgecolors='k',alpha=target_alpha,marker="^",s=50)
            else :
                axs[1].scatter(X_t[:, 0], X_t[:, 1], c="grey", norm=norm, edgecolors='k',alpha=target_alpha,marker="^",s=50)
                legend_elements.append(Line2D([0],[0],marker='.',ls="",markeredgecolor="grey",markerfacecolor='grey',label='Unknown class label',alpha=target_alpha))
        else :
            if target_colors :
                y_pred_t = np.argmax(model.predict(X_t)[0],axis=1)
                axs[1].scatter(X_t[:, 0], X_t[:, 1], c=y_t-y_pred_t, cmap="bwr", norm=norm, edgecolors='k',alpha=target_alpha,marker="^",s=50)
            else :
                axs[1].scatter(X_t[:, 0], X_t[:, 1], c="grey", norm=norm, edgecolors='k',alpha=target_alpha,marker="^",s=50)
                legend_elements.append(Line2D([0],[0],marker='.',ls="",markeredgecolor="grey",markerfacecolor='grey',label='Unknown class label',alpha=target_alpha))
        legend_elements.append(Line2D([0],[0],marker='^',ls="",markeredgecolor="black",markerfacecolor='w',label='Target points',alpha=target_alpha))
    ...
    if settings["background_distribution"]["name"] == settings["signal_distribution"]["name"] == "Gaussian" :
        ...
        if source_alpha > 0 :
            axs[1].plot((source_b_center[0],source_s_center[0]),(source_b_center[1],source_s_center[1]), color="black", linestyle="-.", alpha=source_alpha)
        if target_alpha > 0 :
            axs[1].plot((target_b_center[0],target_s_center[0]),(target_b_center[1],target_s_center[1]), color="black", linestyle="-.", alpha=target_alpha)
    ...
    if save_figure :
        ...
        plt.savefig("DANN_visualisation_" + time_string + ".png")
...
def compute_3D_boundary(Z) :
    i_max,j_max,k_max = Z.shape
    Z_out = 2*np.ones((i_max,j_max,k_max)).astype(int)
    for i in range(1,i_max):
        for j in range(1,j_max):
            for k in range(1,k_max):
                main = Z[i,j,k]
                pre_i,pre_j,pre_k = Z[i-1,j,k],Z[i,j-1,k],Z[i,j,k-1]
                Z_out[i-1,j,k] = abs(main-pre_i)
                Z_out[i,j-1,k] = abs(main-pre_j)
                Z_out[i,j,k-1] = abs(main-pre_k)
    return Z_out[0:-1,1:,1:]

def compute_3D_in_between_grid(x_min,x_max,resolution) :
    nb_points = int((x_max-x_min)/resolution)-1
    linspace = np.linspace(x_min+resolution/2, x_max+resolution/-2, nb_points)
    return linspace

def to_ravel_3D(xx,Z) :
    # This function is meant to select only points on the decision boundary
    mask = Z == 1
    return xx[mask]
...
def visualize_3D_DANN_boundaries(model, 
                                 x_source, y_source, x_target, y_target, 
                                 show_boundary=False, show_regions=True,
                                 settings=None, scores=None, nb_region_points=10):
    markers = ('o', 'x')
    colors = ('blue', 'red')
    cmap = ListedColormap(colors)

    # Determine the minimum and maximum values for each feature
    x1_min, x1_max = min(x_source[:, 0].min(),x_target[:, 0].min()) - 1, max(x_source[:, 0].max(),x_target[:, 0].max()) + 1
    x2_min, x2_max = min(x_source[:, 1].min(),x_target[:, 1].min()) - 6, max(x_source[:, 1].max(),x_target[:, 1].max()) + 6
    x3_min, x3_max = min(x_source[:, 2].min(),x_target[:, 2].min()) - 1, max(x_source[:, 2].max(),x_target[:, 2].max()) + 1
    
    resolution1 = (x1_max-x1_min)/nb_region_points
    resolution2 = (x2_max-x2_min)/nb_region_points
    resolution3 = (x3_max-x3_min)/nb_region_points
    # Generate a grid of points with a specified resolution
    xx1, xx2, xx3 = np.meshgrid(np.arange(x1_min, x1_max, resolution1),
                                np.arange(x2_min, x2_max, resolution2),
                                np.arange(x3_min, x3_max, resolution3))

    # Reshape the grid to match the classifier input shape
    grid = np.array([xx1.ravel(), xx2.ravel(), xx3.ravel()]).T

    # Use the trained model to predict the class labels for all points in the grid
    logits = model.predict(grid)[0]
    ...
    if show_regions :
        regions = go.Scatter3d(
            ...
            marker=dict(
                size=10,
                color= Z.ravel(),
                colorscale=[[0, "rgb(229,0,0)"],
                            [1, "rgb(03,43,223)"]],
                opacity=0.2))
        data_to_visualize.append(regions)
    
    # Visualize boundary
    if show_boundary :
        in_between_Z = compute_3D_boundary(Z)
        in_between_xx1, in_between_xx2, in_between_xx3 = np.meshgrid(compute_3D_in_between_grid(x1_min, x1_max, resolution1),
                                                                 compute_3D_in_between_grid(x2_min, x2_max, resolution2),
                                                                 compute_3D_in_between_grid(x3_min, x3_max, resolution3))
        boundary = go.Scatter3d(
            x= to_ravel_3D(in_between_xx1,in_between_Z),
            y= to_ravel_3D(in_between_xx2,in_between_Z),
            z= to_ravel_3D(in_between_xx3,in_between_Z),
            mode='markers',
            marker=dict(
                size=3,
                color= "black",
        ...
        data_to_visualize.append(boundary)
    
    # Visualize source points
    preds_source = np.argmax(model.predict(x_source)[0],axis=1)
    source_points = go.Scatter3d(
        x=x_source[:,0],
        y=x_source[:,1],
        z=x_source[:,2],
        mode='markers',
        marker=dict(
            size=5,
            color=y_source, #y_source-preds_source, #preds_source, # 
            colorscale=[[0.0, "rgb(229,0,0)"],
                        [0.5, "rgb(255,255,255)"],
                        [1.0, "rgb(03,43,223)"]],
            cmin = 0, #-1,
            cmax = 1,
            ...
            symbol='square'),
            name='Source points')
    data_to_visualize.append(source_points)
    
    # Visualize target points
    preds_target = np.argmax(model.predict(x_target)[0],axis=1)
    target_points = go.Scatter3d(
        x=x_target[:,0],
        y=x_target[:,1],
        z=x_target[:,2],
        mode='markers',
        marker=dict(
            size=5,
            color=y_target, # y_target-preds_target,# preds_target, # 
            colorscale=[[0.0, "rgb(229,0,0)"],
                        [0.5, "rgb(255,255,255)"],
                        [1.0, "rgb(03,43,223)"]],
            cmin = 0, #-1,
            ...
            symbol="circle"),
            name="Target points")
    data_to_visualize.append(target_points)
    ...
    fig = go.Figure(data=data_to_visualize, layout=layout)
