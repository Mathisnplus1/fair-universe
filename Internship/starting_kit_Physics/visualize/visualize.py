...
def new_visualize_histogram(result, thetas, show_signal=True, show_background=True):

    # Get train and test predictions and decisions
    train_predictions_list = result['Y_hat_trains']
    test_predictions_list = result['Y_hat_tests']
    train_decisions_list = result['Y_hat_trains_decisions']
    test_decisions_list = result['Y_hat_tests_decisions']

    for theta,train_predictions, test_predictions, train_decisions, test_decisions in zip(thetas, train_predictions_list, test_predictions_list, train_decisions_list, test_decisions_list) :
        # Indexes of signal and background points
...
def visualize_DANN_summary(model) :
    ...
    for i in range(2,20) :
        section_colors[f'domain_clf_{i} '] = fg('hot_pink_1a')
        section_colors[f'label_clf_{i} '] = fg('blue')
...
def visualize_DANN_boundaries(model, X_s, y_s, X_t, y_t, settings, scores,
                              ...
                              target_colors=True, # Plot target points with meaningful colors if True and in grey if False.Note that if y_t is None, then target_colors has no effect. 
    ...
    if source_alpha > 0 :
        if before_training :
            ...
            y_pred_s = model.predict(X_s) # np.argmax(model.predict(X_s)[0],axis=1)
    ...
    if target_alpha > 0 :
        if before_training :
            ...
            if target_colors and y_t is not None:
                y_pred_t = model.predict(X_t) # np.argmax(model.predict(X_t)[0],axis=1)
    ...
    if settings is not None and (settings["background_distribution"]["name"] == settings["signal_distribution"]["name"] == "Gaussian") :
        l = settings["L"]
